...
export class DashboardComponent implements OnInit {
  ...
  constructor(
    ...
    // Se verifica que las variables de sesión sean validas.

    if (
      this.storage.get("usuario") == null ||
      this.storage.get("usuario") == undefined ||
      this.storage.get("usuario") == "" ||
      this.storage.get("token") == null ||
      this.storage.get("token") == undefined ||
      this.storage.get("token") == "" ||
      this.storage.get("residential") == null ||
      this.storage.get("residential") == undefined ||
      this.storage.get("residential") == "" ||
      this.storage.get("token2") == null ||
      this.storage.get("token2") == undefined ||
      this.storage.get("token2") == "" ||
      this.storage.get("speaker") == null ||
      this.storage.get("speaker") == undefined ||
      this.storage.get("observer") == null ||
      this.storage.get("observer") == undefined ||
      this.storage.get("moroso") == null ||
      this.storage.get("moroso") == undefined
    ) {
      sessionStorage.clear();
      this.router.navigate(["login/" + this.residential_id]);
    }

    //Se está guardando la data almacenada en las variables de sesión...
    ...
    // Sí en las variables de sesión, es hablante, se cambia el valor de inIntervetionRoom  a true.
    ...
    // Se ejecuta el servicio que obtiene los detalles de la reunión.
    // se implementara un servicio para la nueva integracion de JWT de jitsi 24/04/2023
    this.httpClient.get(this.config.endpoint4 + "/ApiMeetings/getVideoMeetingToken/" + this.keysession + '/' + this.meeting_id).subscribe(Response => {
      this.newTokenJWT = Response['content']
    })

    //fin de la implmentacion jitsi JWT



    this.httpClient
      .get(
        this.config.endpoint4 +
        "ApiMeetings/getMeetingDetails/" +
        this.keysession +
        "/" +
        this.meeting_id
      )
      .subscribe((resp2) => {
        if (resp2["success"]) {
          ...
          this.rise_hand_open = resp2["content"]["isRaisingHandOpen"];
          if (
            this.youtube_link.match(/www/gi) == null &&
            this.youtube_link.match(/http/gi) == null
          ) {
            this.isTwitch = true;
            setTimeout(() => {
              this.getVolume();
          ...
          this.interval = setTimeout(() => {
            if (
              ...
              document
                .getElementById("youtube_id")
                .setAttribute("src", this.youtube_link);
            } else {
              try {
          ...
          //Obtener las unidades asignadas a ese usuario del usuario.
          this.httpClient
            .get(
            ...
            .subscribe((resp5) => {
              //Si el usuario no tiene unidades asignadas, entonces se cierra su sesión
              if (
                resp5["message"] ==
                "El usuario no tiene propiedades registradas en la base de datos"
              ) {
                this.httpClient
                  .get(
                    this.config.endpoint4 +
                    "ApiUsers/closeSession/" +
                    this.keysession +
                    "/" +
                    this.meeting_id
                  )
                ...
                this.userName =
                  resp5["content"]["nameRegister"] +
                  " " +
                  resp5["content"]["properties"][0]["name"];
              }
            });
        } else {
          const formData = new FormData();
      ...
      this.socketService.listen('listen_session_check_'+this.meeting_id).subscribe(response =>{
        // console.log(response)
          let data = {'meeting_id':this.meeting_id,'session':this.keysession}
          this.socketService.emit('notify_session_confirmed',data)
      })
  }
  ...
  ngOnInit() {
    ...
    this.socketService
      ...
      .listen("check_presence_" + this.keysession)
      .subscribe((response) => {
        if (response["time"] < 60) {
          var message =
            "Para verificar que usted está presente en este momento, haga clic en el botón Confirmar Presencia. <br> Este mensaje se cerrara en: ";
        } else {
          var message =
            "Para verificar que usted está presente en este momento, haga clic en el botón Confirmar Presencia.";
        }
          ...
          .fire({
            ...
            didOpen: () => {
              ...
              if (response["time"] < 60) {
                ...
                var secs = Math.floor(
                  ((response["time"] / 60 - hours) * 60 - min) * 60
                );
    ...
    this.socketService
      .listen("property_request_updated_" + this.keysession)
      .subscribe((response) => {
        var icon = "info";
        var text = "";
        ...
        this.httpClient
          .get(
            this.config.endpoint4 +
            "ApiUsers/getCustomerPropertiesByMeeting/" +
            this.keysession +
            "/" +
            this.meeting_id
          )
          .subscribe((response) => {
            ...
            for (let index = 0; index < this.userunits.length; index++) {
              if (typeof this.userunits[index]["coefficient"] === "string") {
                this.userunits[index]["coefficient"] = parseFloat(
                  this.userunits[index]["coefficient"].replace(/,/, ".")
                );
                this.representedcoefficient +=
                  this.userunits[index]["coefficient"];
              } else {
                this.representedcoefficient +=
                  this.userunits[index]["coefficient"];
              }
    ...
    this.socketService
      .listen("customer_session_" + this.keysession)
      .subscribe((response) => {
        this.global.messageLogin = false;
        swal.fire({
          icon: "info",
          html:
            "Su sesión se ha abierto en otro lugar por " +
            response["user_name"] +
            ", por favor comuníquese con el soporte telefónico si tiene alguna duda Pbx.: (601)4325200 Opción 2",
          showCancelButton: false,
          confirmButtonColor: "#e56e22",
          timer: 20000,
        });
    ...
    this.socketService
      .listen("listen_message_" + this.keysession)
      .subscribe((response) => {
        const formData = new FormData();
        formData.append("key", this.config.key);
        formData.append("id", this.user_id);
        ...
        this.httpClient
          ...
          .subscribe((user) => {
            ...
            swal.fire({
              icon: "info",
              html: "Su sesión ha sido finalizada 2",
              showCancelButton: false,
              confirmButtonColor: "#e56e22",
              timer: 20000,
            });
    ...
    this.socketService
      .listen("participation_request_" + this.keysession)
      .subscribe((response) => {
        ...
        if (this.inIntervetionRoom == false) {
          if (response["status"] == 1) {
            ...
            this.raisehand_status_button = 0
            this.rise_hand_open = true
            this.accepted = 0
          }
          this.accepted = "0";
          this.storage.set("accepted", "0");
        }
        if(this.isSpeaker == 1 && response["status"] !=''){
          this.raisehand_status_button = 1;
    ...
    this.socketService.listen("voting_activated_" + this.meeting_id).subscribe((response3) => {
      ...
      // sessionStorage.setItem("active_voted", "1")
      if (this.observer == 0) {
        this.router.navigate(["/home/votaciones2/"]);0
        this.callModalVote();
        this.customObservable2.emit(response3);
      } else {
        this.router.navigate(["/home/resultados2/" + response3['id']]);
        swal.fire({
    ...
    this.socketService
      .listen("check_meeting_time_" + this.meeting_id)
      .subscribe((response3) => {
        if (response3["hours"] == "d") {
          ...
          document.getElementById("container_timer").style.visibility =
            "hidden";
          if (this.isTwitch) {
            document.getElementById("container_timer_button").style.display =
              "none";
          }
        } else {
          clearInterval(this.interval3);
    ...
    this.socketService
      .listen("meeting_finished_" + this.meeting_id)
      .subscribe((resp) => {
        swal.fire({
          title: "Atención",
          html: resp["message"],
    ...
    this.socketService
      .listen("voting_finished_" + this.meeting_id)
      .subscribe((resp) => {
        this.modalActive = false;
        // sessionStorage.removeItem("active_voted")
        this.myModal.hide();
        swal.fire({
          title: "Atención",
          html: resp["message"],
      ...
      // Sockets de la nueva peticion de palabra
    this.socketService.listen("toggle_raising_hand_button_" + this.meeting_id).subscribe((resp) => {
      // console.log(resp)
      this.rise_hand_open = toBoolean(resp['show'])
      if (this.rise_hand_open == false && this.accepted == 3) {
        this.rise_hand_open = true;
      }
    });
 
    this.socketService.listen("toggle_raising_hand_button_session_"+ this.keysession).subscribe((resp) => {
      this.rise_hand_open = toBoolean(resp['show'])
    });
  ...
  show(){
    // console.log('rise_hand_open: ',this.rise_hand_open)
    //  console.log('raisehand_status_button: ',this.raisehand_status_button)
    //  console.log('raisehand_status: ',this.raisehand_status)
  ...
  raiseHand(accepted) {
    if(accepted == 4){
      swal.fire({
        title: "Le informamos que",
        html: 'En este momento no hay intervenciones activas, cuando se activen aparecerá el siguiente ícono <img src="./assets/img/hand.png" style="height: 30px; display: inline-block;" alt="Mano"> en este lugar',
        icon: "info"
      });
    }else{
    // if (sessionStorage.getItem("active_voted")) {
    //   swal
    //     .fire({
    //       title: "Lo sentimos",
    //       text: 'Durante una votacion en curso no es posible pedir la palabra',
    //       confirmButtonColor: "#e56e22",
    //       confirmButtonText: "Ok",
    //       icon: 'error'
    //     })
    // } else {
      ...
      this.httpClient
        .get(
          this.config.endpoint4 +
          "ApiRaisingHands/storeRaisingHands/" +
          this.keysession +
          "/" +
          this.meeting_id +
          "/" +
  ...
  youtube() {
    ...
    document
      ...
      .setAttribute("src", this.youtube_link + "?playsinline=1&autoplay=1");
  }
  ...
  Delete(unitId, index) {
    swal
      .fire({
        title:
          "¿Está seguro de que desea renunciar a la representación de esta unidad?",
        showDenyButton: true,
          ...
          this.httpClient
            ...
            .subscribe((response) => {
              if (response["success"]) {
                this.representedcoefficient = 0;
                swal.fire(
                  "Unidad eliminada de su representación",
                  "",
                  "success"
                );
                ...
                for (let index = 0; index < this.userunits.length; index++) {
                  if (
                    typeof this.userunits[index]["coefficient"] === "string"
                  ) {
                    this.userunits[index]["coefficient"] = parseFloat(
                      this.userunits[index]["coefficient"].replace(/,/, ".")
                    );
      ...
      .subscribe((response) => {
        if (response["success"]) {
          swal.fire({
            ...
            title: "¡Atención!",
            text:
              "Se ha enviado una solicitud para adicionar la unidad " +
              sector.name +
              " " +
              sector.number +
              " " +
              ...
              " " +
              unit.number +
              " a su representación. En un momento recibirá un mensaje con la respuesta a su solicitud.",
            confirmButtonColor: "#e56e22 ",
            confirmButtonText: "Ok",
            timer: 10000,
          });
          ...
          if (
            response["message"] ==
            "No es posible recibir su petición, se ha alcanzado el límite permitido."
          ) {
            swal.fire({
              icon: "question",
              title: "¡Atención!",
  ...
  navigate(url) {
    if (this.router.url != url) {
      ...
      if (
        document.getElementById("votacionReload") &&
        url == "home/votacion2"
      ) {
  ...
  ngOnDestroy() {
    ...
    this.socketService.removeListen(
      "property_request_updated_" + this.keysession
    );
    ...
    this.socketService.removeListen("toggle_raising_hand_button_" + this.meeting_id);
  }
  ...
  openModalAddunits() {
    this.httpClient
      .get(
        this.config.endpoint4 +
        "ApiUnits/getBuildingsUnitByMeeting/" +
        this.keysession +
        "/" +
        this.meeting_id
      )
