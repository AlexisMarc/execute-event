  ...
  signCustomer(datosUsuarios, name, document, id_conjunto) {
    ...
    this.httpClient.get(this.config.endpoint4 + 'ApiMeetings/getMeetingDetailsByResidential/' + id_conjunto)
      .subscribe(response => {
        ...
        dataToSend2.append('name',name)
        dataToSend2.append('document_number',datosUsuarios.get('document_number'))
        dataToSend2.append('password',datosUsuarios.get('password'))
        dataToSend2.append('meeting_id',this.meeting_id)
        this.httpClient.post(this.config.endpoint4 + 'ApiUsers/signInUser',dataToSend2).subscribe((response2) => {
